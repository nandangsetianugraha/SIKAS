...
$data['title'] = 'Laporan Sisa Tunggakan';
  ...
  $(document).ready(function() {
	  $(document).on('click', '#getLaporan', function(e){
        ...
        var smt=$('#smt').val();
				$.ajax({
					type : 'GET',
					url : '../modul/keuangan/laporan-sisa-tunggakan.php',
					data :  'kelas=' + kelas+'&bulan='+bulan+'&jenis='+jenis+'&tapel='+tapel+'&smt='+smt,
					beforeSend: function()
					{	
		...
		$(document).on('click', '#cetaklaporan', function(e){
			...
			var tapel=$('#tapel').val();
          var smt=$('#smt').val();
			PopupCenter('../cetak/cetak-sisa-tunggakan.php?kelas='+kelas+'&bulan='+bulan+'&jenis='+jenis+'&tapel='+tapel+'&smt='+smt, 'Cetak Invoice',800,800);
