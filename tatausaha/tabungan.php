				...
				<div class="col-6">
					<div class="card">
						...
						<?php $jprinter=$connect->query("select * from printer where status='1'")->fetch_assoc(); ?>
							...
							<input type="hidden" name="txtPdfFiled" id="txtPdfFiled" value="../cetak/cetak-all-tabungan.pdf" />
							...
							<div class="card">
								<div class="card-header">
								  ...
								  <div class="card-header-form cetaktotal">
									<a href="#tambahlain" class="btn btn-primary btn-icon" data-toggle="modal" id="cetakallModal"><i class="fas fa-print"></i> Cetak</a>
								  </div>
								</div>
								<div class="card-body">
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                <h4 class="modal-title">Tanggal Transaksi</h4>
              </div>
                       
							<div class="modal-body">
								<div class="form-group form-group-default">
									<label>Tanggal Awal</label>
									<input type="hidden" name="idnasabah" class="form-control" value="<?php echo $idnasabah;?>">
									<input type="text" class="form-control datepicker" name="awal" id="awal">
								</div>
								<div class="form-group form-group-default">
									<label>Tanggal Akhir</label>
									<input type="text" class="form-control datepicker" name="akhir" id="akhir">
								</div>
							</div>
							<div class="modal-footer">
								<button type="button" class="btn btn-default" data-dismiss="modal">Tutup</button>
								<button type="button" class="btn btn-danger" id="cetak-all">Cetak</button>
							</div>
...
<script type="text/javascript">
	...
	function printall() {
        if (jspmWSStatus()) {
            ...
            //Set PDF file
            var my_file = new JSPM.PrintFilePDF($('#txtPdfFiled').val(), JSPM.FileSourceType.URL, 'MyFile.pdf', 1);
            my_file.printRotation = JSPM.PrintRotation[$('#lstPrintRotation').val()];
            my_file.printRange = $('#txtPagesRange').val();
            my_file.printAnnotations = $('#chkPrintAnnotations').prop('checked');
            my_file.printAsGrayscale = $('#chkPrintAsGrayscale').prop('checked');
	...
	function printc() {
        if (jspmWSStatus()) {
            ...
            //Set PDF file
            var my_file = new JSPM.PrintFilePDF($('#txtPdfFilec').val(), JSPM.FileSourceType.URL, 'MyFile.pdf', 1);
            my_file.printRotation = JSPM.PrintRotation[$('#lstPrintRotation').val()];
            my_file.printRange = $('#txtPagesRange').val();
            my_file.printAnnotations = $('#chkPrintAnnotations').prop('checked');
            my_file.printAsGrayscale = $('#chkPrintAsGrayscale').prop('checked');
	...
	$(document).ready(function() {
		...
		$(document).on('click', '#printinv', function(e){
			...
			.then((value) => {
			  ...
			  $.ajax({
					type : 'GET',
					url : '../cetak/cetak-tabungan1.php',
					data :  'idinv='+idinv+'&idcetak='+value+'&idsaldo='+idsaldo,
					success: function (data) {
						printc();
						$("#idNasabah").focus();												
					}
		...
		$('#tanggal').change(function(){
			...
			$(document).on('click', '#cetak-all', function(e){
		
			e.preventDefault();
			
			var awal = $('#awal').val();
			var akhir = $('#akhir').val();
			var idnasabah = $('#idNasabah').val();
			swal("Nomor Pencetakan:", {
			  content: "input",
			  closeOnClickOutside: false,
			})
			.then((value) => {
			  ...
			  $.ajax({
				type : 'GET',
				url : '../cetak/cetak-all-tabungan.php',
				data :  'idnas='+idnasabah+'&awal='+awal+'&akhir='+akhir+'&idcetak='+value,
				success: function (data) {
				printall();
				$("#idNasabah").focus();												
				}
