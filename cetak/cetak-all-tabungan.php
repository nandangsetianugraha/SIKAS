...
$jprinter=$connect->query("select * from printer where status='1'")->fetch_assoc();
		...
		$saldodulu=$connect->query("select sum(masuk) as setor, sum(keluar) as ambil from tabungan where nasabah_id='$idnas' and tanggal < '$awal'")->fetch_assoc();
		$saldo=$saldodulu['setor']-$saldodulu['ambil'];
		$sql11="select * from tabungan where nasabah_id='$idnas' and tanggal >= '$awal' and tanggal <= '$akhir' order by tanggal asc";
		$query11 = $connect->query($sql11);
		while($h=$query11->fetch_assoc()){
			$table2->rowStyle('min-height:5');
			$table2->easyCell($idcetak,'font-size:6;valign:M;align:L');
		...
		$table2->endTable();
		$pdf->Output('F','cetak-all-tabungan.pdf');
