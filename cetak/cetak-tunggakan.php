...
$bln = array("Juli", "Agustus", "September", "Oktober", "November", "Desember", "Januari", "Februari", "Maret", "April", "Mei", "Juni");
		...
		$namakelas=$connect->query("select * from penempatan where peserta_didik_id='$siswa' and tapel='$tapel' and smt='$smt'")->fetch_assoc();
		...
		$table2->printRow();
		$table2->endTable();
		$idnas=$connect->query("select * from nasabah where user_id='$siswa'")->fetch_assoc();
		...
		$table2=new easyTable($pdf, '{80,60}', 'align:L');
		$table2->easyCell('ID Nasabah','align:L;border:BT');
		$table2->easyCell('Saldo Tabungan','align:L;border:BT');
		$table2->printRow();
		$table2->easyCell($idn,'align:L;border:BT');
		$table2->easyCell(rupiah($saldo),'align:L;border:BT');
		$table2->printRow();
		$table2->endTable();
