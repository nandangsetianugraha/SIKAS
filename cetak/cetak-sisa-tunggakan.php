		...
		if($kelas==0){
			...
			while($m=$query1->fetch_assoc()) {
				...
				while($n=$query2->fetch_assoc()) {
					...
					if($jenis==0){
						if($idtung==1){
							$jumlahtunggakan=$connect->query("select sum(tarif) as jumlahspp from penempatan left join tarif_spp on penempatan.peserta_didik_id=tarif_spp.peserta_didik_id where penempatan.rombel='$idromb' and penempatan.tapel='$tapel' and penempatan.smt='$smt'")->fetch_assoc();
							$totaltunggakan=$jumlahtunggakan['jumlahspp']*$bulan;
							$sql5="select sum(bayar) as dibayar from pembayaran left join penempatan on penempatan.peserta_didik_id=pembayaran.peserta_didik_id where penempatan.rombel='$idromb' and penempatan.tapel='$tapel' and penempatan.smt='$smt' and pembayaran.tapel='$tapel' and pembayaran.jenis='$idtung' and pembayaran.bulan <= '$bulan'";
							$query5= $connect->query($sql5);
							$jumlahbayar=$query5->fetch_assoc();
						}else{
							$jumlahtunggakan=$connect->query("select sum(tarif) as jumlahspp from penempatan left join tunggakan_lain on penempatan.peserta_didik_id=tunggakan_lain.peserta_didik_id where penempatan.rombel='$idromb' and penempatan.tapel='$tapel' and penempatan.smt='$smt' and tunggakan_lain.tapel='$tapel' and tunggakan_lain.jenis='$idtung'")->fetch_assoc();
