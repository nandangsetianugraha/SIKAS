...
$jprinter=$connect->query("select * from printer where status='1'")->fetch_assoc();
		...
		$datatab=$connect->query("select * from tabungan where id='$idinv'")->fetch_assoc();
		//$h=$connect->query("select * from tabungan where id='$idtrans'")->fetch_assoc();
		//$saldo=$datatab['setor']-$datatab['ambil'];
		for ($x = 1; $x <= $idcetak; $x++) {
			if($x==$idcetak){
				$table2->rowStyle('min-height:5');
				$table2->easyCell($x,'valign:M;font-size:6;align:L');
				$table2->easyCell($datatab['tanggal'],'valign:M;align:L');
				$table2->easyCell($datatab['kode'],'valign:M;align:C');
				$table2->easyCell(rupiah($datatab['masuk']),'valign:M;align:R');
				$table2->easyCell(rupiah($datatab['keluar']),'valign:M;align:R');
				$table2->easyCell(rupiah($idsaldo),'valign:M;align:R');
		...
		$table2->endTable();
		$pdf->Output('F','cetak-tabungan1.pdf');
