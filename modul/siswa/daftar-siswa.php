...
$tapel=$_GET['tapel'];
	...
	while ($row = $query->fetch_assoc()) {
		...
		$nis=$row['nis'];
		$kelas = $connect->query("select * from penempatan where peserta_didik_id='$idp' and tapel='$tapel' and smt='$smt'")->fetch_assoc();
		...
		$actionButton = '
			...
			<button class="btn btn-sm btn-danger" data-pdid="'.$idp.'" data-tapel="'.$tapel.'" id="cetakDepan"><i class="fas fa-print"></i> C</button>
			<button class="btn btn-sm btn-danger" data-pdid="'.$idp.'" data-tapel="'.$tapel.'" id="cetakBelakang"><i class="fas fa-print"></i> B</button>
			';
