...
while ($row = $query->fetch_assoc()) {
	...
	$tombol='<button class="btn btn-sm btn-danger" data-ids="'.$idtab.'" id="gethapus"><i class="fa fa-trash"></i></button>';
	$output['data'][] = array(
		$namanya,
		$row['kode'],
		rupiah($row['masuk']),
...
$saldo=$connect->query("select sum(masuk) as setor, sum(keluar) as ambil from tabungan where tanggal='$hariini'")->fetch_assoc();
$output['data'][] = array(
		'Total','',rupiah($saldo['setor']),rupiah($saldo['ambil']),''
	);
